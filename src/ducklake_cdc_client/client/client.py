...
``DDLConsumer``, ``StdoutSink``, etc.) and uses this module internally.
    ...
    def cdc_ddl_changes_read(
        ...
        *,
        ...
        named = _read_named_parameters(
            ...
            end_snapshot=end_snapshot,
            auto_commit=auto_commit,
            function_name="cdc_ddl_changes_read",
        )
        rows = self._call("cdc_ddl_changes_read", name, named=named).list()
        return [SchemaChangeRow.from_row(row) for row in rows]
